<script lang="ts">
	...
	import { UserMngr, type UserWithCompletedLessonsCount } from "$lib/user-mngr";
    ...
    onMount(async ()=>{
        user = await userMngr.getUserWithCompletedLessonsCount();
        user.creation_date = new Date(user.creation_date)
    });
    
    let user: UserWithCompletedLessonsCount= {
        username: "Loading...",
        email: "",
        password: "",
        id: 0,
    ...
    <div class=" px-10 w-full grid  m-2 p-2 bg-(--sec-col) rounded-2xl shadow-2xl md:w-2/3">
        <div class="grid grid-cols-1 gap-10">
            ...
            <div class="grid grid-cols-3">
                ...
                <div class="grid grid-cols-1 border p-4 rounded-3xl mx-5 align-middle">
                    <h1 class="text-2xl">{user.completed_lessons}</h1>
                    <h2 class="">пройдено вами</h2>
                </div>
            </div>
        </div>
